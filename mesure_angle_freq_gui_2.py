    ...
    def run(self):
        ...
        while self.ThreadActive:
            ...
            while True:
                ...
                cv2.putText(image, ts, (20, image.shape[0]-20),
                            cv2.FONT_HERSHEY_SIMPLEX, 0.5, green, 1)
                # display FPS
                fps = str(int(RECORD_FPS))
                cv2.putText(image, fps, (20, 40),
                            cv2.FONT_HERSHEY_SIMPLEX, 1, red, 2)
                cv2.putText(image, "fps", (65, 40),
                            cv2.FONT_HERSHEY_SIMPLEX, 1, red, 2)
            ...
            prev_cX = 0
            prev_cY = 0
            # uncomment if no target to film
            ...
            for (i, imagePath) in enumerate(sorted(imagePaths, key=get_number)):
                ...
                if frame is not None:
                    ...
                    cv2.putText(image, real_fps, (20, 50),
                                cv2.FONT_HERSHEY_SIMPLEX, 1, red, 2)
                    cv2.putText(image, "fps", (65, 50),
                                cv2.FONT_HERSHEY_SIMPLEX, 1, red, 2)
                    # display frequency
                    freq_string = str(freq)
                    cv2.putText(image, freq_string, (image.shape[1]-140, image.shape[0]-60),
                                cv2.FONT_HERSHEY_SIMPLEX, 1, deepskyblue, 2)
                    cv2.putText(image, "Hz", (image.shape[1]-70, image.shape[0]-60),
                                cv2.FONT_HERSHEY_SIMPLEX, 1, deepskyblue, 2)
                    # display angle
                    angle_string = str(int(angle))
                    cv2.putText(image, angle_string, (image.shape[1]-140, image.shape[0]-20),
                                cv2.FONT_HERSHEY_SIMPLEX, 1, deepskyblue, 2)
                    cv2.putText(image, "deg", (image.shape[1]-70, image.shape[0]-20),
                                cv2.FONT_HERSHEY_SIMPLEX, 1, deepskyblue, 2)
                    # update progress bar
...
class App(QDialog):
    ...
    def createButtonsLayout(self):
        ...
        self.controlPanelLayout.setContentsMargins(0,0,0,0)
